...
class TTSEngine:
    ...
    def get_available_voices(self) -> List[Dict[str, str]]:
        ...
        available_voices = []
        for voice in voices:
            available_voices.append({"id": voice.id, "name": voice.name, "lang": voice.languages})
        return available_voices

    def set_voice_properties(self, rate: int = None, volume: float = None, pitch: float = None, voice_id: str = None):
        """设置语速、音量、音高（如果支持）、可选voice_id"""
        if voice_id:
            self.engine.setProperty('voice', voice_id)
        
        current_voice_id = self.engine.getProperty('voice') # Get current voice after potential change
        ...
        if pitch is not None:
            ...
            try:
                ...
                print(f"Warning: Could not set pitch directly: {e}")
                # If direct pitch setting fails, we might try to adjust rate for a similar effect
                # For simplicity, we'll just note it for now.
    ...
    def speak_with_emotion(self, text: str, sentiment: Dict[str, float], config: Dict = None):
        ...
        if classification == 'positive':
            ...
            volume_factor = 1.1 # Slightly louder
            pitch_factor = 1.1 # Slightly higher pitch (conceptual)
        elif classification == 'negative':
            rate_factor = 0.9  # Slightly slower
            volume_factor = 0.9 # Slightly softer
            pitch_factor = 0.9 # Slightly lower pitch (conceptual)
        else: # neutral
            rate_factor = 1.0
            volume_factor = 1.0
            pitch_factor = 1.0
        ...
        # Conceptual pitch adjustment - actual effect depends on engine support
        ...
        self.set_voice_properties(rate=self.default_rate, volume=self.default_volume, pitch=self.default_pitch)
