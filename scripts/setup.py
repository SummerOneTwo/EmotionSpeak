...
def create_directories():
    """创建必要的目录"""
    print("正在创建项目目录...")
    directories = ["data/output", "data/sample_texts", "logs"]
...
def setup_spacy_models():
    ...
    try:
        ...
        print(f"❌ spaCy 模型下载失败: {e}")


def setup_transformers_models():
    """下载 transformers 英文情感分析模型"""
    try:
        from transformers import pipeline

        print("正在下载 transformers 英文情感分析模型...")
        _ = pipeline('sentiment-analysis', model='distilbert-base-uncased-finetuned-sst-2-english')
        print("✅ transformers 模型下载完成")
    except Exception as e:
        print(f"❌ transformers 模型下载失败: {e}")
...
def main():
    ...
    try:
        ...
        print("2. 运行 'make web' 启动网页端 (推荐)")
        print("3. 运行 'make demo' 查看演示")
        print("4. 运行 'make test' 执行测试")
        print("5. 查看 README.md 了解更多信息")
